...
function deserialize(buffer, classType, options) {
    ...
    if (!options?.unchecked && reader._offset !== buffer.length) {
        throw new error_js_1.BorshError(`Unexpected ${buffer.length - reader._offset} bytes after deserialized data`);
    }
    return result;
}
...
function deserializeField(fieldName, fieldType, fromBuffer) {
    try {
        ...
        if (fieldType instanceof types_js_1.OptionKind) {
            ...
            return (reader, options) => {
                return reader.u8() ? fieldHandle(reader, options) : undefined;
            };
        }
